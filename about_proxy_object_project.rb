...
class Proxy
  ...
  def method_missing(method_name, *args, &block)
    @message << method_name.to_sym
    if @frequency[method_name.to_sym] == nil
      @frequency[method_name.to_sym] = 0
    end
    @frequency[method_name.to_sym] += 1
  ...
  def called?(method_name)
    return @message.include?(method_name)
  end

  def number_of_times_called(method_name)
    freq = @frequency[method_name]
    return freq == nil ? 0 : freq
  end
end
